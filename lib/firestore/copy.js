 ...
 * @param {string} streamToken the client has received.
 * @return {Promise} a promise for copy.
 */
// TODO(zxu): Really we should use batchGet. For now, use get to make this
// happen in a hackweek.
FirestoreCopy.prototype._copyOneDocument = function(source, target, streamId, streamToken) {
  // Get document
  ...
  return api
    ...
    .then(function(res) {
      ...
      // Write document
      var url = self.parent + ":write";

      var body = {
        "writes": [{
          "update": {"name": self.parent + "/" + target,
                     "fields": {"foo" : "bar"}}, //res.body.fields}
        }],
      }
      if (streamId) {
