 ...
 */
// TODO(zxu123): Really we should use batchGet. For now, use get to make this
 ...
 */
// TODO(zxu123): Really we should use write. For now, use create to make this
// happen in a hackweek.
  ...
  return self._getOneDocument(source)
    ...
    .then(function() {
      if (self.docs[target]) {
        ...
        // No duplicate. Just create a new doc.
...
FirestoreCopy.prototype.execute = function() {
  if (this._isDocumentPath(this.source)) {
    // Copy document (and sub-collections).
    if (this._isCollectionPath(this.target)) {
      var segments = this.source.split("/");
      var documentId = segments[segments.length - 1];
      this.target = this.target + "/" + documentId;
    }
    // TODO(zxu123): dealing with sub-collections.
    return this._copyOneDocument(this.source, this.target);
  } else {
    // TODO(zxu123): Copy collection.
  }
};
